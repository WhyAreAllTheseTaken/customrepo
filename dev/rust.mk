...
RUSTUP_REVISION = 16
RUSTUP_PACKAGE = rustup_$(RUSTUP_VERSION)-$(RUSTUP_REVISION)_all
...
$(REPO)/$(RUSTUP_PACKAGE).deb:
	...
	cp dev/rustup_postinst.sh $(RUSTUP_PACKAGE)/DEBIAN/postinst
	chmod +x $(RUSTUP_PACKAGE)/DEBIAN/postinst
	cp dev/rustup_prerm.sh $(RUSTUP_PACKAGE)/DEBIAN/prerm
	chmod +x $(RUSTUP_PACKAGE)/DEBIAN/prerm
	cp dev/rustup_control $(RUSTUP_PACKAGE)/DEBIAN/control
	dpkg-deb --build $(RUSTUP_PACKAGE)
	mv $(RUSTUP_PACKAGE).deb $(REPO)
	rm -rf $(RUSTUP_PACKAGE)

RUST_CHAIN_VERSION = 1.83.0
RUST_CHAIN_REVISION = 9
RUST_CHAIN_PACKAGE = rust-toolchain_$(RUST_CHAIN_VERSION)-$(RUST_CHAIN_REVISION)_all
...
$(REPO)/$(RUST_CHAIN_PACKAGE).deb:
	...
	cp dev/rust-toolchain_postinst.sh $(RUST_CHAIN_PACKAGE)/DEBIAN/postinst
	chmod +x $(RUST_CHAIN_PACKAGE)/DEBIAN/postinst
	cp dev/rust-toolchain_prerm.sh $(RUST_CHAIN_PACKAGE)/DEBIAN/prerm
	chmod +x $(RUST_CHAIN_PACKAGE)/DEBIAN/prerm
	cp dev/rust-toolchain_control $(RUST_CHAIN_PACKAGE)/DEBIAN/control
	dpkg-deb --build $(RUST_CHAIN_PACKAGE)
	mv $(RUST_CHAIN_PACKAGE).deb $(REPO)
	rm -rf $(RUST_CHAIN_PACKAGE)

RUST_NIGHTLY_VERSION = 1.85.0
RUST_NIGHTLY_REVISION = 5
RUST_NIGHTLY_DATE = 20241127
RUST_NIGHTLY_HASH = 6b6a867ae
RUST_NIGHTLY_PACKAGE = rust-toolchain-nightly_$(RUST_NIGHTLY_VERSION)+git$(RUST_NIGHTLY_DATE)$(RUST_NIGHTLY_HASH)-$(RUST_NIGHTLY_REVISION)_all
...
$(REPO)/$(RUST_NIGHTLY_PACKAGE).deb:
	...
	cp dev/rust-nightly_postinst.sh $(RUST_NIGHTLY_PACKAGE)/DEBIAN/postinst
	chmod +x $(RUST_NIGHTLY_PACKAGE)/DEBIAN/postinst
	cp dev/rust-nightly_prerm.sh $(RUST_NIGHTLY_PACKAGE)/DEBIAN/prerm
	chmod +x $(RUST_NIGHTLY_PACKAGE)/DEBIAN/prerm
	cp dev/rust-nightly_control $(RUST_NIGHTLY_PACKAGE)/DEBIAN/control
...
MIRI_DATE = 20241127
MIRI_HASH = 6b6a867ae
MIRI_PACKAGE = cargo-miri_$(MIRI_VERSION)+git$(MIRI_DATE)$(MIRI_HASH)-$(MIRI_REVISION)_all
...
$(REPO)/$(MIRI_PACKAGE).deb:
	...
	cp dev/miri_postinst.sh $(MIRI_PACKAGE)/DEBIAN/postinst
	chmod +x $(MIRI_PACKAGE)/DEBIAN/postinst
	cp dev/miri_prerm.sh $(MIRI_PACKAGE)/DEBIAN/prerm
	chmod +x $(MIRI_PACKAGE)/DEBIAN/prerm
	cp dev/miri_control $(MIRI_PACKAGE)/DEBIAN/control
